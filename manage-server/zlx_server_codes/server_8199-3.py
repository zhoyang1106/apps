...
def update_leatest_connector():
    global LEATEST_CONNECTOR_WORKER
    while True:
        LEATEST_CONNECTOR_WORKER = min(enumerate(worker.processing_cnt for worker in WORKERS), key=lambda x:x[1])[0]
        time.sleep(1)
